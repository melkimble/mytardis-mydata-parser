                ...
                if not rta_complete:
                    ...
                    if extra_backup_dirs:
                        for extra_backup_dir in extra_backup_dirs:
                            output_backup_dir = extra_backup_dir + self.backup_parsed_dir_name + project + "/" + \
                                                run_id + "/"
        ...
        try:
            ...
            for index, row in dirs_group_df.iterrows():
                ...
                if extra_backup_dirs:
                    for extra_backup_dir in extra_backup_dirs:
                        # copy into each extra backup directory
                        output_backup_dir = extra_backup_dir + self.backup_original_dir_name + project + "/" + \
                                            run_id + "/"
    ...
    def complete_upload_backup(self, dirs_df, upload_parsing, move_parsing, move_staging):
        ...
        try:
            ...
            if move_parsing and move_staging:
                ...
                api_logger.info('Backup made of parsed and original [' + backup_parsed_dir + ', ' +
                                backup_original_dir + ']')
...
class GenericParser(MiSeqParser):
    def __init__(self, project, run_dir, num_run_dir,
        ...
        super().__init__(project, run_dir, num_run_dir, staging_dir, output_dir, backup_dir,
                         extra_backup_dirs, create_fastq_sid_dirs,
                         log_file_dir, data_directory, folder_structure, mytardis_url)
        self.staging_dir = staging_dir
        self.backup_dir = backup_dir
        self.backup_original_dir_name = "Original/"
        self.backup_parsed_dir_name = "Parsed/"
